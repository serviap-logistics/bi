...
export default async function airtableRequest(endpoint: string, settings: airtable_request_type){
  const records_found : any[] = []
  try{
    do{
      const page = await fetch(`${endpoint}${generateEncode(settings)}`, airtable_request_options).then((res) => res.json())
      if(page?.records) {
        const records: any[] = page.records.map(({id, createdTime, fields}) => ({
