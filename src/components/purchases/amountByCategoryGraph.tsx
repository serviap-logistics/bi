...
export default function PurchasesAmountsByCategoryGraph( props : {purchases : purchase_type[] }) {
  ...
  const [charts, setCharts] = useState({
    ...
    'Travel Bar' : {
      ...
      options: {
        ...
        tooltip: {enabled: true, y: { formatter: (val) => USDollar.format(val)}}
      },
      series: [
        {name: '', data: []}
      ]
  ...
  const updateResults = (purchases : purchase_type[]) => {
    ...
    const travel_purchases = Object.entries(purchases_by_category).filter(([name, _]) => name.includes('Travel'))
    const tools_purchases  = Object.entries(purchases_by_category).filter(([name, _]) => name.includes('Tools') )
    ...
    setCharts(({
      ...
      'Travel': {
        ...charts['Travel'],
        options: {
          labels : travel_purchases.map(([name,_]) => name.replace('Travel - ', ''))
        },
        series: travel_purchases.map(([_, purchases] : [string, any]) => (
          purchases.reduce((subtotal: any, purchase: purchase_type) => subtotal + purchase.total_cost,0)
        )),
      },
      'Travel Bar': {
        ...charts['Travel Bar'],
      ...
      'Tools': {
        ...charts['Tools'],
        options: {
          labels : tools_purchases.map(([name,_]) => name.replace('Tools - ', ''))
        },
