          ...
          <li className="px-4 py-4 sm:px-6 flex grap-y-4 justify-evenly">
            <>
              ...
              <div>
                ...
                <p className="text-lg text-gray-600">
                  { USDollar.format(
                    (costAnalysis?.total_cost ?? 0) - purchases.reduce((total, purchase) => total + purchase.total_cost, 0)
                    ) }
                </p>
              </div>
              <div>
                ...
                <p className="text-lg text-gray-600">
                  { 
                    ...
                    ?
                      <Toast
                        text={
                          ((purchases.reduce((total, purchase) => total + purchase.total_cost, 0)*100)/costAnalysis.total_cost).toFixed(2) + '%'
                        }
                        text_size="text-base"
                        color={
                          ((purchases.reduce((total, purchase) => total + purchase.total_cost, 0)*100)/costAnalysis.total_cost) <= 50 ? 'success'
                        : ((purchases.reduce((total, purchase) => total + purchase.total_cost, 0)*100)/costAnalysis.total_cost) <= 70 ? 'info'
                        : ((purchases.reduce((total, purchase) => total + purchase.total_cost, 0)*100)/costAnalysis.total_cost) <= 90 ? 'warning'
                        : 'error'
                        }
                      />
                    : 
                      <Toast
                        ...
                        text_size="text-base"
                        color={
                          purchases.reduce((total, purchase) => total + purchase.total_cost, 0) > 0 ? 'error' : 'success'
                        }
                      />
                  }
                </p>
