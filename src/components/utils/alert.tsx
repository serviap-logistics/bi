...
export default function Alert(props: {
  status: 'success' | 'info' | 'warning' | 'error', label: string, details?: string | string[],
  buttons?: [{label: string, action: any}]
}) {
  const {status, label, details, buttons} = props;
  const status_color = status === 'success' ? 'success'
                     : status === 'info' ? 'blue'
                     : status === 'warning' ? 'warning'
                     : status === 'error' ? 'red' : 'secondary'
    ...
    <div className={`rounded-md bg-${status_color}-200 p-4`}>
      <div className="flex">
        <div className="flex-shrink-0">
          {
            status === 'error' &&
            <XCircleIcon aria-hidden="true" className={`h-5 w-5 text-${status_color}-400`} />
          }
        </div>
        <div className="ml-3">
          <h3 className={`text-sm font-medium text-${status_color}-800`}>{label}</h3>
