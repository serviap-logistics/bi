...
export default function HeadcountProjectDetails(){
  ...
  const [budget, setBudget] = useState<report_data>({ hours: 0, cost: 0, people: 0 })
  const [real, setReal] = useState<report_data>({ hours: 0, cost: 0, people: 0 })
  ...
  const updateReal = async (project_id) => {
    ...
    const total_employees = [... new Set(real_times.map((record) => record.employee_id))].length
  ...
  return (
    <div className="overflow-hidden bg-white sm:rounded-md">
      <ul role="list" className="divide-y divide-gray-200 flex flex-col align-middle">
        <li className="px-4 py-4 sm:px-6 lg:px-10 grid grid-cols-2 grap-y-4 gap-x-8">
          ...
          <div className="text-center grid grid-cols-2">
            <p className="col-span-2 font-semibold tracking-tight text-gray-900">People Quantity</p>
            <div>
              <p className="tracking-tight text-gray-900">Expected</p>
              <p> <Toast text='TO DEFINE' text_size="text-base" color='error' /> </p>
            </div>
            <div>
              <p className="tracking-tight text-gray-900">Registered</p>
              <p> <span className="text-lg font-semibold">{real.people}</span> labors</p>
            </div>
