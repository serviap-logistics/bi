...
import { cloneObject, getDateByTimestamp, getDatesBeetween, groupListBy } from "../../utils";
...
const empty_cell_data : cell_data = {
  hour_cost: 0, hours: 0, people_quantity: 0, subtotal: 0
}
...
export default function HeadcountTableByRole() {
  ...
  const [reportDates, setReportDates] = useState<{
    start_date: string | undefined, end_date: string | undefined, dates_beetween: string[] | undefined
  }>()
  ...
  const updateReportDates = () => {
    ...
    if(start_date && end_date){
      setReportDates({
        start_date: start_date, end_date: end_date, dates_beetween: getDatesBeetween(start_date, end_date)
      })
    }
  }
  ...
  const updateBudget = async () => {
    if(!costAnalysis?.cost_analysis_id || !reportDates?.start_date || !reportDates?.end_date) {
      ...
      Object.entries(grouped_by_role).map(([role, records] : [string, any]) => {
        worked_by_role[role] = {}
        records.map((record) => (worked_by_role[role][record.date] = {
          date: record.date,
      ...
      Object.entries(grouped_by_role).map( ([role, records] : [string, any]) => {
        ...
        records.map((record) => (worked_overtime_by_role[role][record.date] = {
          date: record.date,
          people_quantity: record.people_quantity,
      ...
      Object.entries(grouped_by_role).map( ([role, records] : [string, any]) => {
        travel_by_role[role] = {}
        records.map((record) => (travel_by_role[role][record.date] = {
          date: record.date,
          people_quantity: record.people_quantity,
      ...
      Object.entries(grouped_by_role).map(([role, records] : [string, any]) => {
        ...
        records.map((record) => (travel_overtime_by_role[role][record.date] = {
          date: record.date,
          people_quantity: record.people_quantity,
  ...
  const mergeResults = () => {
    if (!reportDates?.start_date || !reportDates.end_date || !reportDates.dates_beetween){
      ...
      if(roles_available.length === 0){
        ...
        for(let group of groups_available){
          for(let role of roles_available){
            // Budgets
            if(budgets[group] && budgets[group][role]){
              const dates : string[] = Object.keys(budgets[group][role])
              const dates_to_create = reportDates.dates_beetween.filter((date) => !dates.includes(date))
              dates_to_create.map((date) => {
                budget_results[group][role][date] = {date: date, ...empty_cell_data}
              })
            }
          }
        }
        const budget_formatted_results: result_data = {};
        Object.entries(budget_results).map(([group, data_by_role]:[string, any]) => ( budget_formatted_results[group] = [
  ...
  const formatAsTable = (results : result_data) => {
    ...
    setColumns(columns)
    setRows(Object.entries(results).map(([group, rows]) => ({name: group, rows: rows})))
    setShowTable(true)
  }

  useEffect(() => {
    let resultsCheck: boolean = Object.values(results).flatMap((element) => element.length > 0).includes(true);
    if(resultsCheck) formatAsTable(results);
  }, [results])

  useEffect(() => {
    let budgetsCheck: boolean = Object.values(budgets).flatMap((element) => Object.keys(element).length > 0).includes(true)
    if( budgetsCheck ){ mergeResults() }
  }, [budgets])

  useEffect(() => {
    let realsCheck: boolean = true
    ...
    <div className="mx-auto max-w-full relative">
      {
        ...
        <Table
          ...
          styles={{
            ...
            static_headers: false, max_height: 'max-h-[27rem]'
          }}
        />
      }
      {
